...
def save_and_visu(image, results, config):
	...
	det_save_path=os.path.join(config['det_save_dir'], image_name[0:-4]+'.txt')
	...
	for result in results:
		...
		if (config['visu_detection'] and (score>0.7)):
...
for root, dirs, files in os.walk(os.path.join(root_dir,'image_1000/')):
	    ...
	    config['det_visu_path']='./demo_images/'+image_name
